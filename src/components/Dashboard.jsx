...
import { Shield, Activity, LogOut, BarChart2, Settings, Users } from 'lucide-react';
      ...
      <header className="flex-none min-h-[4rem] py-3 sm:py-0 border-b border-white/5 bg-white/5 backdrop-blur-md flex flex-wrap items-center justify-between px-4 sm:px-6 z-10 gap-3">
        ...
        <div className="flex items-center gap-2 sm:gap-4 w-full sm:w-auto justify-between sm:justify-end">
          ...
          {user?.role === 'admin' && (
            <button
              onClick={onOpenAdmin}
              className="flex items-center gap-2 px-2.5 py-1.5 sm:px-3 sm:py-1.5 rounded-lg border border-emerald-500/20 bg-emerald-500/10 hover:bg-emerald-500/20 transition-colors text-xs sm:text-sm text-emerald-400 font-medium"
            >
