...
const MetricsPanel = ({ angles, risk }) => {
    ...
    return (
        <aside className="w-full h-full flex flex-col gap-6 overflow-y-auto pr-2 custom-scrollbar">
            {/* Risk Card */}
            <div className={`relative p-8 rounded-3xl border border-white/5 bg-slate-900/40 backdrop-blur-xl shadow-[0_8px_32px_rgba(0,0,0,0.4)] overflow-hidden transition-all duration-700 group hover:bg-slate-900/60 hover:border-white/10`}>
                {/* Glow effects */}
                <div className={`absolute -right-12 -top-12 w-48 h-48 rounded-full ${risk.statusColor} opacity-20 blur-[50px] group-hover:opacity-40 transition-opacity duration-700`}></div>
                <div className={`absolute -left-12 -bottom-12 w-48 h-48 rounded-full ${risk.statusColor} opacity-10 blur-[50px] group-hover:opacity-30 transition-opacity duration-700`}></div>

                <div className="relative z-10">
                    <h3 className="text-xs font-bold text-slate-400 uppercase tracking-[0.2em] mb-4">Nivel de Riesgo</h3>
                    <div className="flex items-baseline gap-2 mb-4">
                        <span className={`text-7xl font-black tracking-tighter ${risk.color} drop-shadow-[0_0_20px_rgba(255,255,255,0.1)] transition-colors duration-500`}>{risk.score}</span>
                        <span className="text-xl text-slate-500 font-light">/ 10</span>
                    </div>
                    <div className={`inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-xs font-bold text-white border border-white/10 backdrop-blur-md bg-white/5 shadow-inner`}>
                        <div className={`w-2 h-2 rounded-full ${risk.statusColor} shadow-[0_0_8px_currentColor]`}></div>
                        <span className={risk.color}>{risk.level.toUpperCase()}</span>
                    </div>
                </div>
            </div>
            ...
            <div className="p-6 rounded-3xl bg-slate-900/40 backdrop-blur-xl border border-white/5 shadow-xl transition-all duration-500 hover:bg-slate-900/60 hover:border-white/10 flex-1">
                <div className="flex items-center gap-3 mb-8">
                    ...
                    <h3 className="text-sm font-bold text-slate-200 uppercase tracking-wider">Eje Central</h3>
                </div>
